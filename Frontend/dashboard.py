...
#     "CO_COL": "CO Quality",
#     "uv_index": "UV Index 🔆"          # <<< Added for UV display & styling
# }
...
# ------------------------------------------- Updated code with uv_index, pressure, altitude fetch ---------------------------------------------------------------
...
THRESHOLDS = {
    ...
    "AQI": 150,               # AQI threshold (unhealthy)
    "soil_moisture_low": 20,  # soil moisture low threshold (percent)
    "CO_Quality": 50          # CO quality threshold
}
...
DISPLAY_NAMES = {
    ...
    "CO_COL": "CO Quality",
    "uv_index": "UV Index 🔆",
    "pressure_altitude": "Altitude (BPM)"
}
...
dark_theme = """
<style>
html, body, [data-testid="stAppViewContainer"] {background-color: #0d0d0d !important; color: #ffffff !important;}
[data-testid="stMarkdown"] {color: #ffffff !important;}
h1, h2, h3, h4, h5, h6, p, label, span, div {color: #ffffff !important;}
.sidebar .sidebar-content {background-color: #0d0d0d !important; color: white !important;}
[data-testid="stDataFrame"] div {color: white !important;}
[data-testid="stMetricValue"] {color: white !important;}
[data-testid="stMetricLabel"] {color: #cccccc !important;}
.stButton>button {background-color: #222 !important; color: white !important; border-radius: 8px; border: 1px solid #444;}
.stButton>button:hover {background-color: #333 !important; border-color: #888;}
.stAlert {background-color: #331111 !important; color: white !important;}
table, th, td {color: white !important;}
iframe {border: 3px solid #00b894 !important;}
...
st.markdown("""
<h1 style='text-align: center; color: white;'>🛡️ TERRA WATCH CONNECT 🛡️</h1>
<h2 style='text-align: center; color: #cccccc; margin-top: -10px;'>Live Environment & Threat Intelligence</h2>
<p style='text-align: center; color: #aaaaaa; font-size: 18px;'>
TerraWatch Connect is an advanced IoT-driven monitoring platform...
</p>
""", unsafe_allow_html=True)
...
while True:
    try:
        ...
        if not df.empty:
            ...
            # ----------------- ALERT CHECKS -----------------
            try:
                if "temperature" in latest and float(latest["temperature"]) > THRESHOLDS["temperature"]:
                    alert_messages.append("🔥 Temperature too high")
            except: pass

            try:
                if "humidity" in latest and float(latest["humidity"]) < THRESHOLDS["humidity_low"]:
                    alert_messages.append("💧 Humidity too low")
            except: pass

            try:
                if "pressure" in latest and (float(latest["pressure"]) < THRESHOLDS["pressure_low"] or float(latest["pressure"]) > THRESHOLDS["pressure_high"]):
                    alert_messages.append("⚠️ Pressure out of range")
            except: pass

            try:
                if "uv_index" in latest and float(latest["uv_index"]) > THRESHOLDS["uv_index"]:
                    alert_messages.append("☀️ High UV exposure")
            except: pass
            ...
            for aqi_col in ["Air_Purity", "air_quality", "AQI"]:
                if aqi_col in latest:
                    try:
                        if float(latest[aqi_col]) > THRESHOLDS["AQI"]:
                            alert_messages.append(f"🏭 AQI high ({latest[aqi_col]}) — poor air quality")
                    except: pass

            # CO alert
            try:
                if CO_COL in latest and float(latest[CO_COL]) > THRESHOLDS["CO_Quality"]:
                    alert_messages.append(f"⚠️ CO levels high ({latest[CO_COL]})")
            except: pass

            # Soil moisture alert
            try:
                if SOIL_COL in latest and float(latest[SOIL_COL]) < THRESHOLDS["soil_moisture_low"]:
                    alert_messages.append(f"🌱 Soil moisture low ({latest[SOIL_COL]})")
            except: pass
                ...
                for val in col_series:
                    ...
                    try:
                        if orig_col == "temperature" and float(val) > THRESHOLDS["temperature"]:
                            ...
                            style = "background-color: #ffea00; color: black;"
                        elif orig_col in ["Air_Purity","air_quality","AQI"] and float(val) > THRESHOLDS["AQI"]:
                            style = "background-color: #7b1fa2; color: white;"
                        elif orig_col == CO_COL and float(val) > THRESHOLDS["CO_Quality"]:
                            style = "background-color: #d32f2f; color: white;"
                        elif orig_col == SOIL_COL and float(val) < THRESHOLDS["soil_moisture_low"]:
            ...
            with placeholder.container():
                ...
                with header_col1:
                    st.subheader("Live Telemetry Snapshot")
                with header_col2:
                    last_update_str = pd.to_datetime(latest.get("timestamp", time.time())).strftime("%Y-%m-%d %H:%M:%S")
                    st.metric("Last update", last_update_str)
                ...
                with center1:
                    r1c1, r1c2, r1c3, r1c4 = st.columns(4)
                    r1c1.metric("🌡 Temp(DHT11)", f"{latest.get('temperature','-')} °C")
                    r1c2.metric("💧 Humidity(DHT11)", f"{latest.get('humidity','-')} %")
                    r1c3.metric("⏲ Pressure(BMP)", f"{latest.get('pressure','-')} hPa")
                    r1c4.metric("🔆UVIndex(UV_Sensor)", f"{latest.get('uv_index','-')}")

                # ROW 2
                pad2, center2, pad2b = st.columns([1,2,1])
                with center2:
                    r2c1, r2c2, r2c3, r2c4 = st.columns(4)
                    r2c1.metric("🏭 CO(MQ7)(0–500)", latest.get("CO_Quality","-"))
                    r2c2.metric("🌱 Soil M(Cap Soil Moisture)", latest.get("soil_moisture","-"))
                    r2c3.metric("🚨 Shock(Gun Sensor)", latest.get("gunfire","-"))
                    r2c4.metric("🚶PIR Motion Sensor)", latest.get("pir_motion","-"))
                ...
                if {"temperature","humidity","pressure"}.issubset(plot_df.columns):
                    with chart_col_left:
                        st.subheader("Environmental Trends")
                        st.line_chart(plot_df[["temperature","humidity","pressure"]].apply(pd.to_numeric, errors="coerce").dropna(how="all"))
                if SOIL_COL in plot_df.columns:
                    with chart_col_mid:
                        st.subheader("Soil Moisture")
                        st.line_chart(plot_df[[SOIL_COL]].apply(pd.to_numeric, errors="coerce").dropna())
                if CO_COL in plot_df.columns:
                    with chart_col_right:
                        st.subheader("CO Quality")
                        st.line_chart(plot_df[[CO_COL]].apply(pd.to_numeric, errors="coerce").dropna())
                # Binary sensors
                present_binary_cols = [c for c in BINARY_COLS if c in plot_df.columns]
                if present_binary_cols:
                    st.subheader("Event Sensors (0 / 1) — recent timeline")
